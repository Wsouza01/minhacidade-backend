...
export const loginRoute = (app) => {
    app.post("/login", {
        ...
        try {
            const { login, senha, tipo } = request.body;
            const agora = new Date();
            // Primeiro tenta buscar por login ou email
            let usuario = await db
                ...
                .then((res) => res[0]);
            // Se não encontrou por login/email, verifica se é CPF
            if (!usuario) {
                const cleanLogin = login.replace(/\D/g, "");
                if (cleanLogin.length === 11) {
                    // Pode ser CPF, busca todos os usuários e verifica hash
            ...
            const tipoValido = usuario.usu_tipo === tipo ||
