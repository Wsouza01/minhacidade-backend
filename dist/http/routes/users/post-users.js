...
export const postUsersRoute = (app) => {
    app.post("/users", {
        schema: {
            body: z.object({
                nome: z
        ...
        try {
            ...
            const cidadeExistente = await db
                .select()
                .from(cidades)
                .where(and(eq(cidades.cid_id, endereco.cidadeId), eq(cidades.cid_ativo, true)))
                .then((res) => res[0] || null);
            if (!cidadeExistente) {
            ...
            const cpfCriptografado = await hashCPF(cpfFormatado);
            const [novoUsuario] = await db
                .insert(usuarios)
                .values({
                usu_nome: nome,
                usu_cpf: cpfCriptografado,
                usu_email: email,
