...
export const getSacByCidadeRoute = async (app) => {
    app.get('/sac-ouvidoria/cidade', {
        schema: {
            description: 'Recupera mensagens de SAC/Ouvidoria para a cidade do administrador.',
            tags: ['SAC/Ouvidoria'],
        ...
        try {
            ...
            if (!admin || !admin.cityId) {
                ...
                return reply.status(403).send({
                    message: 'Acesso negado. Rota disponível apenas para administradores de cidade.',
                });
            }
            ...
            const mensagens = await db
                ...
                .where(and(eq(usuarios.cid_id, admin.cityId), isNotNull(sacOuvidoria.usu_id)))
                .orderBy(desc(sacOuvidoria.sac_data_criacao));
            return reply.status(200).send(mensagens);
        }
        catch (error) {
            console.error('Erro ao buscar mensagens da ouvidoria por cidade:', error);
