...
export const postResolverChamado = async (app) => {
    app.post('/chamados/:id/resolver', {
        schema: {
            params: z.object({
                id: z.string().uuid(),
            }),
        ...
        try {
            ...
            const atendentes = chamado.cha_departamento
                ? await db
                    ...
                    .from(funcionarios)
                    .where(and(eq(funcionarios.dep_id, chamado.cha_departamento), eq(funcionarios.fun_tipo, 'atendente'), eq(funcionarios.fun_ativo, true)))
                : [];
            if (atendentes.length > 0) {
                const notificacoesAtendentes = atendentes.map((atendente) => ({
                    ...
                    not_mensagem: resolvido
                        ? `Servidor ${servidor.fun_nome} resolveu o chamado #${chamado.cha_id.slice(0, 8)}. ${observacao}`
                        : `Servidor ${servidor.fun_nome} atualizou o chamado #${chamado.cha_id.slice(0, 8)}. ${observacao}`,
                    not_tipo: 'info',
                    not_lida: false,
                    not_data: new Date(),
                    cha_id: id,
