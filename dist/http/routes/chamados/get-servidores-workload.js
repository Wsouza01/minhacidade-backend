    ...
    app.get('/servidores/workload', {
        ...
        try {
            ...
            const servidoresData = await db
                ...
                .from(funcionarios)
                .where(and(eq(funcionarios.cid_id, cidadeId), eq(funcionarios.fun_tipo, 'servidor')));
            // Para cada servidor, contar chamados abertos
            const servidoresComWorkload = await Promise.all(servidoresData.map(async (servidor) => {
                const [_resultado] = await db
                    .select()
                    .from(chamados)
                    .where(and(eq(chamados.cha_responsavel, servidor.fun_id), isNull(chamados.cha_data_fechamento)));
