        ...
        try {
            ...
            if (cidadeIdDoDepto) {
                const [atendenteEncontrado] = await db
                    .select()
                    .from(funcionarios)
                    .where(and(eq(funcionarios.cid_id, cidadeIdDoDepto), eq(funcionarios.fun_tipo, 'atendente'), eq(funcionarios.fun_ativo, true)))
                    .limit(1);
            ...
            if (atendenteDaCidade) {
                try {
                    ...
                    console.log(`✅ Notificação criada para o atendente da cidade: ${atendenteDaCidade.fun_id}`);
                }
                catch (atendenteNotifError) {
                    console.error('❌ Erro ao criar notificação para o atendente da cidade:', atendenteNotifError);
                }
            }
