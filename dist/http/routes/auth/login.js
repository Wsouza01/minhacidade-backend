...
export const authLoginRoute = async (app) => {
    app.post("/auth/login", {
        ...
        try {
            ...
            // Helper para verificar se login pode ser CPF
            const cleanLogin = login.replace(/\D/g, "");
            const isPossibleCPF = cleanLogin.length === 11;
            // Primeiro tenta encontrar como administrador (admin global ou admin de cidade)
            let administrador = await db
                .select()
                .from(administradores)
                .where(or(eq(administradores.adm_login, login), eq(administradores.adm_email, login)))
                .then((res) => res[0]);
            // Se não encontrou e login pode ser CPF, verifica hash
            if (!administrador && isPossibleCPF) {
                const allAdmins = await db.select().from(administradores);
                for (const admin of allAdmins) {
                    const match = await verifyCPF(cleanLogin, admin.adm_cpf);
                    if (match) {
                        administrador = admin;
                        break;
                    }
            ...
            let usuario = await db
                .select()
                .from(usuarios)
                .where(or(eq(usuarios.usu_login, login), eq(usuarios.usu_email, login)))
                .then((res) => res[0]);
            // Se não encontrou e login pode ser CPF, verifica hash
            if (!usuario && isPossibleCPF) {
                const allUsers = await db.select().from(usuarios);
                for (const user of allUsers) {
                    const match = await verifyCPF(cleanLogin, user.usu_cpf);
                    if (match) {
                        usuario = user;
                        break;
                    }
            ...
            let funcionario = await db
                ...
                .from(funcionarios)
                .where(or(eq(funcionarios.fun_login, login), eq(funcionarios.fun_email, login)))
                .then((res) => res[0]);
            // Se não encontrou e login pode ser CPF, verifica hash
            if (!funcionario && isPossibleCPF) {
                const allFuncionarios = await db.select().from(funcionarios);
                for (const func of allFuncionarios) {
                    const match = await verifyCPF(cleanLogin, func.fun_cpf);
                    if (match) {
                        funcionario = func;
                        break;
                    }
