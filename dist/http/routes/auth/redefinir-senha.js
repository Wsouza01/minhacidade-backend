...
export async function redefinirSenhaRoute(app) {
    app.withTypeProvider().post('/auth/redefinir-senha', {
        schema: {
            tags: ['auth'],
            ...
            body: z.object({
                ...
                novaSenha: z
                    ...
                    .regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]/, 'A senha deve conter pelo menos: 1 letra maiúscula, 1 minúscula, 1 número e 1 caractere especial'),
            }),
            response: {
                200: z.object({
                    message: z.string(),
        ...
        try {
            ...
            const tokenEncontrado = await db
                ...
                .where(and(eq(tokensRecuperacao.tok_token, token), isNull(tokensRecuperacao.tok_usado), // Não foi usado ainda
