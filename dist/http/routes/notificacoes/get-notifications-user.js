...
export const getNotificationsUserRoute = (app) => {
    ...
    app.get('/notificacoes/funcionario/:funcionarioId', async (request, reply) => {
        try {
            ...
            console.log('[NOTIFICACOES] Buscando notificações para funcionário:', funcionarioId);
            const funcionarioNotifications = await db
                ...
                .orderBy(desc(notificacoes.not_data));
            console.log('[NOTIFICACOES] Notificações encontradas:', funcionarioNotifications.length);
            console.log('[NOTIFICACOES] Dados:', JSON.stringify(funcionarioNotifications, null, 2));
