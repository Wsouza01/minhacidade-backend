...
export const deleteFuncionariosRoute = (app) => {
    app.delete('/funcionarios/:id', {
        ...
        try {
            ...
            if (errorObj?.cause?.code === '23503' ||
                errorObj?.message?.includes('foreign key constraint') ||
                errorObj?.message?.includes('violates foreign key')) {
                return reply.status(400).send({
                    message: 'Não é possível remover este funcionário porque ele possui registros vinculados (chamados atribuídos, etc). Remova primeiro os registros relacionados.',
