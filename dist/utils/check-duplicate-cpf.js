...
export async function checkDuplicateCPF(cpf, excludeId) {
    ...
    for (const admin of admins) {
        if (excludeId && admin.adm_id === excludeId)
            continue;
    ...
    for (const func of funcs) {
        if (excludeId && func.fun_id === excludeId)
            continue;
    ...
    for (const user of users) {
        if (excludeId && user.usu_id === excludeId)
            continue;
...
export async function getCPFDuplicateMessage(cpf, excludeId) {
    ...
    const admins = await db
        ...
        .from(administradores);
    for (const admin of admins) {
        if (excludeId && admin.id === excludeId)
            continue;
    ...
    const funcs = await db
        ...
        .from(funcionarios);
    for (const func of funcs) {
        if (excludeId && func.id === excludeId)
            continue;
