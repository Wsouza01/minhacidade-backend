...
export const getFuncionariosByDepartamentoRoute: FastifyPluginCallbackZod = (app) => {
  app.get<{ Params: z.infer<typeof getFuncionariosByDepartamentoParamsSchema> }>(
    ...
    async (request, reply) => {
      try {
        ...
        reply.code(500).send({
          ...
          message: error instanceof Error ? error.message : "Erro ao buscar funcionários do departamento",
        })
      }
    },
  )
