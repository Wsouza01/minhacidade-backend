    ...
    async (request, reply) => {
      try {
        ...
        const [departamento, categoria, usuario, anexosList] = await Promise.allSettled([
          ...
          // Categoria
          chamado.cat_id ?
            db.select({ cat_nome: categorias.cat_nome })
              .from(categorias)
        ...
        const depData = departamento.status === 'fulfilled' && departamento.value.length > 0 ? departamento.value[0] : null;
        const catData = categoria.status === 'fulfilled' && categoria.value.length > 0 ? categoria.value[0] : null;
        ...
        reply.status(500).send({
          ...
          error: process.env.NODE_ENV === 'development' ? (err instanceof Error ? err.message : 'Erro desconhecido') : undefined
        });
      }
    }
  );
