			...
			try {
				...
				if (period === "hoje") {
					...
					const hoursData = await db
						.select({
							hour: sql<number>`EXTRACT(HOUR FROM timezone(${timezone}, ${chamados.cha_data_abertura}))::int`,
						...
						.where(
							and(
								sql`DATE(timezone(${timezone}, ${chamados.cha_data_abertura})) = DATE(timezone(${timezone}, now()))`,
								cityFilter,
							),
						)
						.groupBy(
							sql`EXTRACT(HOUR FROM timezone(${timezone}, ${chamados.cha_data_abertura}))`,
						)
						.orderBy(
							sql`EXTRACT(HOUR FROM timezone(${timezone}, ${chamados.cha_data_abertura}))`,
						);
					...
					const daysData = await db
						.select({
							date: sql<string>`DATE(timezone(${timezone}, ${chamados.cha_data_abertura}))`,
							count: sql<number>`count(*)::int`,
						})
						.from(chamados)
						.leftJoin(
						...
						.where(
							and(
								sql`timezone(${timezone}, ${chamados.cha_data_abertura}) >= date_trunc('day', timezone(${timezone}, now()) - interval '6 days')`,
								cityFilter,
							),
						)
						.groupBy(
							sql`DATE(timezone(${timezone}, ${chamados.cha_data_abertura}))`,
						)
						.orderBy(
							sql`DATE(timezone(${timezone}, ${chamados.cha_data_abertura}))`,
						);
					...
					const daysData = await db
						...
						.where(
							and(
								sql`timezone(${timezone}, ${chamados.cha_data_abertura}) >= date_trunc('day', timezone(${timezone}, now()) - interval '29 days')`,
								cityFilter,
							),
						)
						.groupBy(
