...
export const getAdministradoresRoute: FastifyPluginCallback = (app) => {
  app.get(
    ...
    async (request, reply) => {
      try {
        const query = request.query as { cidadeId?: string; ativo?: string }
        ...
        if (cidadeId) {
          queryBuilder = queryBuilder.where(
            eq(administradores.cid_id, cidadeId),
          ) as any
        }
        if (ativo !== undefined) {
          queryBuilder = queryBuilder.where(
            eq(administradores.adm_ativo, ativo === 'true'),
          ) as any
        }
        ...
        return reply.send({
          ...
          dataNascimento:
            typeof row.dataNascimento === 'string'
              ? row.dataNascimento
              : (row.dataNascimento as Date).toISOString().split('T')[0],
          login: row.login,
          ativo: row.ativo,
          criado: row.criado,
          cidade: row.cidadeId
