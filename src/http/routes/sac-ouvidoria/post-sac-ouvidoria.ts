...
export const postSacOuvidoriaRoute: FastifyPluginCallback = (app) => {
	app.post("/sac-ouvidoria", async (request, reply) => {
		try {
			...
			const descricao = (data.fields.descricao as any)?.value as string;
			const usuario_id = (data.fields.usuario_id as any)?.value as string;
